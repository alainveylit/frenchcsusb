	...
	$.fn.crossword = function(entryData) {
			...
			*/
				...
				buildEntries: function() {
					...
					for (var x=1, p = entryCount; x <= p; ++x) {
						...
						for (var i=0; i < entries[x-1].length; ++i) {
							...
							var light=undefined;
							//var td = '[data-coords="' + entries[x-1][i] + '"]';
							$( puzzCells ).each(function( index ) {
							...
							/*$(puzzCells).each( function(index, elem) {
								console.log(this.attr('data-coord');
							});*/
			...
			var nav = {
				...
				updateByEntry: function(e, next) {
					...
					if(e.keyCode === 9 || next){
						...
						util.getActivePositionFromClassGroup(e.target);
						clue=util.findClue(clueLiEls, activePosition);
			...
			var util = {
				...
				findClue: function (clueLiEls, data_position) {
					var elem;
					$(clueLiEls).each( function( index) {
						  if($( this ).attr('data-position')==data_position) {
								  elem= $( this ); return;
							  }
						  });
					return elem;
				...
				highlightClue: function() {
					var clue;				
					$('.clues-active').removeClass('clues-active');
					var active_clue=this.findClue(clueLiEls, activePosition);
					if(active_clue) active_clue.addClass('clues-active');
					//$(clueLiEls + '[data-position=' + activePosition + ']').addClass('clues-active');
					
					if (mode === 'interacting') {
						var active_clue=this.findClue(clueLiEls, activePosition);
						//clue = $(clueLiEls + '[data-position=' + activePosition + ']');
						if(active_clue) activeClueIndex = $(clueLiEls).index(active_clue);
					};
				},
				...
				getActivePositionFromClassGroup: function(el){
						...
						if(classes.length > 1){
							...
							class_type_0 = classes[0].split('-')[1];
							class_type_1 = classes[1].split('-')[1];
							
							e1Ori = this.findClue(clueLiEls, class_type_0).parent().prop('id');
							e2Ori = this.findClue(clueLiEls, class_type_1).parent().prop('id');
							/* breaks
