<div class="fatty index">
<div>
	<h2><?php echo __('Chansons'); ?></h2>
		<table class="table table-striped">
			<thead>
			<tr>
					<th><?php echo $this->Paginator->sort('titre'); ?></th>
					<th><?php echo $this->Paginator->sort('auteur'); ?></th>
					<th><?php echo $this->Paginator->sort('created'); ?></th>
					<th class="actions"><?php echo __('Actions'); ?></th>
			</tr>
			</thead>
			<tbody>
			<?php foreach ($chansons as $chanson): ?>
			<tr>
				<td><?php echo $this->Html->link($chanson['Chanson']['titre'], array('action' => 'view', $chanson['Chanson']['id'])); ?>&nbsp;</td>
				<td><?php echo h($chanson['Chanson']['auteur']); ?>&nbsp;</td>
				<td><?php echo h($chanson['Chanson']['created']); ?>&nbsp;</td>
				<td class="actions">
					<?php echo $this->Html->link(__('View'), array('action' => 'view', $chanson['Chanson']['id'])); ?>
					<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $chanson['Chanson']['id'])); ?>
					<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $chanson['Chanson']['id']), array('confirm' => __('Are you sure you want to delete # %s?', $chanson['Chanson']['id']))); ?>
				</td>
			</tr>
		<?php endforeach; ?>
			</tbody>
		</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
		'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="pager">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ' | '));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
	</div>
	
	<div class="actions">
		<h3><?php echo __('Actions'); ?></h3>
		<ul class="list-inline"
			<li><?php echo $this->Html->link(__('Ajouter une chanson'), array('action' => 'add')); ?></li>
		</ul>
	</div>
</div>
